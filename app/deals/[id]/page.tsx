...
const ProductPage: React.FC<productIdParams> = async ({ params }) => {
  ...
  try {
    ...
    const productData: ProductData = responseData;
    return (
      <>
        <div className="max-w-7xl mx-auto py-20">
          ...
          <div className="flex flex-col md:flex-row gap-5">
            <div className="flex flex-col w-1/2">
              <div className="flex mt-5 flex-col gap-5 w-full">
                <Image
                  className="w-full min-w-80 border rounded-lg"
                  width={3840}
                  height={2160}
                  src={productData.thumbnail}
                  alt={productData.title}
                />
                <div className="flex flex-wrap justify-normal md:justify-between gap-y-5 px-1 md:px-0 w-full">
                  {productData.images.map((image, index) => (
                    <Image
                      key={index}
                      className="max-w-36 min-w-20 border rounded-lg"
                      width={3840}
                      height={2160}
                      src={image}
